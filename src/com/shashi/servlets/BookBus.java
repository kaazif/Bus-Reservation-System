	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		try {
			...
			if (train != null) {
				...
				if (seat > avail) {
					...
					if (ResponseCode.SUCCESS.toString().equalsIgnoreCase(responseCode)) {
						...
						pw.println("<div class='tab'>" + "<p class='menu'>" + "<table>"
								+ "<tr><td>Candidate No: </td><td colspan='3' style='color:blue;'>" + transaction.getTransId()
								+ "</td></tr><tr><td>Bus Name: </td><td>" + train.getTr_name()
								+ "</td><td>Bus No: </td><td>" + transaction.getTr_no()
								+ "</td></tr><tr><td>Booked From: </td><td>" + transaction.getFrom_stn()
								+ "</td><td>To Depot: </td><td>" + transaction.getTo_stn() + "</td></tr>"
				...
				pw.println("<div class='tab'><p1 class='menu'>Invalid Bus Number !</p1></div>");
